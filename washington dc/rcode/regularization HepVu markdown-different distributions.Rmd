...
#dcRawHepvu$NOD_Rate_2 <- dcRawHepvu$NOD_Rate_2 / 100
```

```{r}
# Plot a histogram of the data
ggplot(dcRawHepvu, aes(x = NOD_Rate_2)) +
  geom_histogram(binwidth = 6, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Histogram of the Data")
```
```{r}
# Perform a goodness-of-fit test (e.g., chi-squared test)
#poisson_fit <- fitdistr(dcRawHepvu$NOD_Rate_2, "Poisson")
#expected_values <- dpois(dcRawHepvu$NOD_Rate_2, lambda = poisson_fit$estimate)

#chi_squared_test <- chisq.test(table(dcRawHepvu$NOD_Rate_2), p = expected_values)
...
filtered_data2 <- dcRawHepvu %>%
  select(one_of(desired_columns))

filtered_data3 <- dcRawHepvu %>%
  select(one_of(desired_columns))
